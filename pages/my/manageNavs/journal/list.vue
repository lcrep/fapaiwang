	...
	<view>
		<view class="recordFixHeader">
			...
			<view class="recordScreenBox">
				...
				<view class="ScreenBox">
					<picker @change="starPicker" :range="starArr" class="pickers">
		...
		<view class="commRecordList" v-if="total!==0">
			<view class="recordItem" v-for="(item,index) in list" :key="index">
				<view class="recordPersonBox">
					...
					<view class="nowStatus"  @click="gotoProcess(item.visitId)">当前进展</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			getList(startPage){
				...
				that.$api.dailyList(param).then(res => {
					if (res.success) {

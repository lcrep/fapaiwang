	...
	<view>
		<view class="searchBox">
			<view class="searchInputBox">
				...
				<input class="uni-input"  placeholder="搜索标的物名称" confirm-type="search" placeholder-style="font-size:26rpx;color:#B8B8B8;" v-model="searchVal" @confirm="confirmSearch"  @input="realtimeSearch" />
			</view>
		...
		<view class="content">
			<view class="goodsBox" v-if="total!==0">
				<view class="goodsList">
					<view class="goodsItem" v-for="(item,index) in houseList" :key="index" @click="searchHouse(item.paimaiId,item.houseSource,item.title)">
						<view class="goodsPic">
			...
			<view class="noData" v-else>
				...
				<text class="noDataText">无标的物</text>
			</view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			searchHouse(id,houseSource,name){
				const that = this;
				...
				if (prevPage) {
					// #ifdef H5
					prevPage.paimaiId = id;
					prevPage.houseSource = houseSource;
					prevPage.houseName = name
					// #endif
